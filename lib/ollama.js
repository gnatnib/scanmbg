...
async function chatCompletion(messages, { temperature = 0.3, maxTokens = 1200, timeoutMs = 30000 } = {}) {
...
JSON:
{
  ...
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 45000); // 45s timeout for vision
  ...
  try {
    ...
    clearTimeout(timer);
    if (fetchErr.name === "AbortError") {
      throw new Error("Qwen vision timeout after 45s — model mungkin sedang sibuk");
    }
    throw fetchErr;
  } finally {
    clearTimeout(timer);
